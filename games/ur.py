...
while True:
    ...
    if user_input.lower() == "r" or "rules":
        print(
            "Ur is the world's oldest known board game dating back to the mesopetamian region over 4500 years ago."
        )
        print(
            "Each player is given seven pieces which move along the tiles as shown...."
        )
        print("LAYOUT")
        print("| 4 | 3 | 2 | 1 |       | 14| 13|")
        ...
        if rolled == 0:
            ...
            while c < 14:
                if board[player - 1][c] == player and int(c + rolled) > 13:
                    ...
                    board[player - 1][c] == player
                    and board[player - 1][int(c + rolled)] != player
                    and int(c + rolled) != 8
                ):
                    commands.append([player, "m", int(c + 1 + rolled), int(c + 1)])
                    options = options + 1
                    print(
                        str(options) + ") Move piece from",
                        int(c + 1),
                        "to",
                        int(c + 1 + rolled),
                    )
            ...
            if options != 0:
                ...
                if len(commands[user_input]) == 3:
                    board, pieces, home = update_board(
                        board,
                        pieces,
                        home,
                        commands[user_input][0],
                        commands[user_input][1],
                        commands[user_input][2],
                    )
                else:
                    board, pieces, home = update_board(
                        board,
